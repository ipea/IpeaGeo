...
namespace IpeaGEO
{
    ...
    public class clsClusterizacaoEspacialHierarquica
    {
        ...
        public double[,] DadosCategoricosClusterizacao
        {
            ...
            {
                clsUtilTools utl = new clsUtilTools();
                this.dados_categoricos = utl.ArrayDoubleClone(value);
        ...
        public void GeraArvoreContinuousHierarchicalClustering(ref ToolStripProgressBar prBar, ref ToolStripStatusLabel lblEvolucao)
        {
            ...
            if (this.dados == null || this.dados.GetLength(0) == 0 || this.dados.GetLength(1) == 0)
            {
                ...
                if (this.m_tipo_dados_clusterizacao == TipoDadosClusterizacao.Mistos)
                {
                    if (this.dados_binarios.GetLength(0) > 0 && this.dados_categoricos.GetLength(0) > 0 && this.dados_continuos.GetLength(0) > 0)
                    {
                        this.dados = utl.Concateh(utl.Concateh(this.dados_continuos, this.dados_binarios), this.dados_categoricos);
                    }
                    else
                    {
                        if (this.dados_binarios.GetLength(0) > 0 && this.dados_categoricos.GetLength(0) > 0)
                        {
                            this.dados = utl.Concateh(this.dados_binarios, this.dados_categoricos);
                        }
                        else
                        {
                            if (this.dados_continuos.GetLength(0) > 0 && this.dados_categoricos.GetLength(0) > 0)
                            {
                                this.dados = utl.Concateh(this.dados_continuos, this.dados_categoricos);
                            }
                            else
                            {
                                if (this.dados_continuos.GetLength(0) > 0 && this.dados_binarios.GetLength(0) > 0)
                                {
                                    this.dados = utl.Concateh(this.dados_continuos, this.dados_binarios);
                                }
                                else
                                {
                                    if (this.dados_binarios.GetLength(0) > 0) this.dados = utl.ArrayDoubleClone(this.dados_binarios);
                                    if (this.dados_categoricos.GetLength(0) > 0) this.dados = utl.ArrayDoubleClone(this.dados_categoricos);
                                    if (this.dados_continuos.GetLength(0) > 0) this.dados = utl.ArrayDoubleClone(this.dados_continuos);
                                }
                            }
                        }
                    }
