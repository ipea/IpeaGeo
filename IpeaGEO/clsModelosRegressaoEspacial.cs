...
namespace IpeaGEO
{
    ...
    public class clsModelosRegressaoEspacial
    {
        ...
        protected clsShape m_shape = new clsShape();
        ...
        {
            this.m_W = new double[this.Shape.Count, this.Shape.Count];
            int[] ind_vizinhos = new int[0];

            for (int i = 0; i < this.Shape.Count; i++)
            {
                ind_vizinhos = this.Shape[i].ListaIndicesVizinhos;
                for (int k = 0; k < ind_vizinhos.GetLength(0); i++)
                {
