...
namespace IpeaGeo.RegressoesEspaciais
{
    ...
    #region Classe MathFdist
    [Obsolete("Use MathNet.Numerics.Distributions instead")]
    public class MathFdist : MathBeta
    {
        private double nu1, nu2;
        private double fac;

        public MathFdist(double nnu1, double nnu2)
        {
            nu1 = nnu1;
            nu2 = nnu2;
            if (nu1 <= 0.0 || nu2 <= 0.0) throw new Exception("bad nu1,nu2 in Fdist");
            fac = 0.5 * (nu1 * Math.Log(nu1) + nu2 * Math.Log(nu2)) + MathSpecialFunctions.gammln(0.5 * (nu1 + nu2))
                - MathSpecialFunctions.gammln(0.5 * nu1) - MathSpecialFunctions.gammln(0.5 * nu2);
        }

        public double p(double f)
        {
            if (f <= 0.0) throw new Exception("bad f in Fdist");
            return Math.Exp((0.5 * nu1 - 1.0) * Math.Log(f) - 0.5 * (nu1 + nu2) * Math.Log(nu2 + nu1 * f) + fac);
        }
        ...
        public double invcdf(double p)
        {
            if (p <= 0.0 || p >= 1.0) throw new Exception("bad p in Fdist");
    ...
    public class MathNormaldist : MathErf
    {
        ...
        public MathNormaldist()
        {
            ...
            mu = mmu;
            sig = ssig;
            if (sig <= 0.0) throw new Exception("bad sig in Normaldist");
        }
        ...
        public double invcdf(double p)
        {
            if (p <= 0.0 || p >= 1.0) throw new Exception("bad p in Normaldist");
