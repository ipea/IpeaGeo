...
namespace IpeaGeo.RegressoesEspaciais
{
    public class clsDBaseHeader
    {
        ...
        private clsDBaseField[] _dbaseColumns;
        ...
        public clsDBaseField[] Columns
        {
            get { return _dbaseColumns; }
            internal set
            {
                _dbaseColumns = value;
                HeaderLength = (short)((clsDBaseConstants.ColumnDescriptionLength * _dbaseColumns.Length) + clsDBaseConstants.ColumnDescriptionOffset + 1 /* For terminator */);
                RecordLength = 1; //Deleted flag length
                Array.ForEach<clsDBaseField>(_dbaseColumns, new Action<clsDBaseField>(delegate(clsDBaseField field) { RecordLength += (short)field.Length; }));
            }
        }
        ...
        {
            return String.Format("[clsDBaseHeader] Records: {0}; Columns: {1}; Last Update: {2}; Record Length: {3}; Header Length: {4}", RecordCount, Columns.Length, LastUpdate, RecordLength, HeaderLength);
        }
        ...
        {
            DataTable schema = clsDBaseSchema.CreateSchemaTable();

            foreach (clsDBaseField dbf in _dbaseColumns)
                schema.Columns.Add(dbf.ColumnName, dbf.DataType);

            for (int i = 0; i < _dbaseColumns.Length; i++)
            {
                DataRow r = schema.NewRow();
                r[clsDBaseSchema.ColumnNameColumn] = _dbaseColumns[i].ColumnName;
                r[clsDBaseSchema.ColumnSizeColumn] = _dbaseColumns[i].Length;
                r[clsDBaseSchema.ColumnOrdinalColumn] = i;
                r[clsDBaseSchema.NumericPrecisionColumn] = _dbaseColumns[i].Decimals;
                r[clsDBaseSchema.NumericScaleColumn] = 0;
                r[clsDBaseSchema.DataTypeColumn] = _dbaseColumns[i].DataType;
                r[clsDBaseSchema.AllowDBNullColumn] = true;
                r[clsDBaseSchema.IsReadOnlyColumn] = true;
                r[clsDBaseSchema.IsUniqueColumn] = false;
                r[clsDBaseSchema.IsRowVersionColumn] = false;
                r[clsDBaseSchema.IsKeyColumn] = false;
                r[clsDBaseSchema.IsAutoIncrementColumn] = false;
                r[clsDBaseSchema.IsLongColumn] = false;
        ...
        internal static clsDBaseHeader ParseDbfHeader(BinaryReader reader)
        {
            clsDBaseHeader header = new clsDBaseHeader();

            if (reader.ReadByte() != clsDBaseConstants.DbfVersionCode)
            ...
            reader.BaseStream.Seek(clsDBaseConstants.EncodingOffset, SeekOrigin.Begin); //Seek to encoding flag
            header.LanguageDriver = reader.ReadByte(); //Read and parse Language driver
            reader.BaseStream.Seek(clsDBaseConstants.ColumnDescriptionOffset, SeekOrigin.Begin); //Move past the reserved bytes

            int numberOfColumns = (storedHeaderLength - clsDBaseConstants.ColumnDescriptionOffset) / clsDBaseConstants.ColumnDescriptionLength;  // calculate the number of DataColumns in the header
            clsDBaseField[] columns = new clsDBaseField[numberOfColumns];
            for (int i = 0; i < columns.Length; i++)
            {
                columns[i] = new clsDBaseField();
                ...
                reader.BaseStream.Seek(clsDBaseConstants.BytesFromEndOfDecimalInFieldRecord, SeekOrigin.Current);
            }
